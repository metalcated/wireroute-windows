...
```powershell
msiexec.exe /i WireRoute-x64-1.1.3.msi /qn /norestart
```

A quiet uninstall can use the deployed MSI or its product code:

```powershell
msiexec.exe /x WireRoute-x64-1.1.3.msi /qn /norestart
```
...
Each profile's On-Demand action offers [Automatic profiles](AUTOMATIC_PROFILES.md), with secondary access in Settings, following WireRoute Android's network-profile switching model. Enabling it pauses the saved single-profile rules. Disabling it does not silently resume them or disconnect the current tunnel. Only connections started by automatic switching in the current app session can be switched; existing or manual connections are never adopted.
