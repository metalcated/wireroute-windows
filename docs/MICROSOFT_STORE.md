...
The output is written below `installer\dist\store\1.1.3.0\development`. An unsigned package cannot be installed directly. Local installation testing requires a development certificate whose subject matches the manifest publisher and whose public certificate is trusted on the test machine.
...
Store-upload output is isolated below `installer\dist\store\1.1.3.0\store-upload`, so it cannot be confused with development-identity packages.
